...
void Renderer_GL::SetBackBufferSize(int w, int h)
{
	backBufferSize = { w, h };
}
...
void Renderer_GL::Init(void* window)
{
...
void Renderer_GL::ClearBackBuffer()
{
...
PerfStats Renderer_GL::GetPerfStats() const
{
	return perfStats;
}
...
u8 Renderer_GL::AddRenderPass(const RenderPass& desc)
{
	renderPasses.push_back(desc);
	return static_cast<u8>(renderPasses.size() - 1);
}

u8 Renderer_GL::AddRenderPass(const char* name, FrameBufferHandle target, ClearColor color, ClearDepth depth)
{
	RenderPass pass{};
	pass.mName = name;
	pass.mTarget = target;
...
u8 Renderer_GL::AddRenderPass(const char* name, ClearColor color, ClearDepth depth)
{
	FrameBufferHandle fb{};
	return AddRenderPass(name, fb, color, depth);
}

ShaderBufferHandle Renderer_GL::CreateShaderBuffer(const void* data, u32 size)
{
...
UniformBufferHandle Renderer_GL::CreateUniformBuffer(const void* data, u32 size)
{
...
void Renderer_GL::UpdateShaderBuffer(const void* data, u32 size, u32 offset, ShaderBufferHandle binding)
{
	UpdateGLBuffer(binding.idx, data, offset, size);
}

void Renderer_GL::UpdateUniformBuffer(const void* data, u32 size, u32 offset, UniformBufferHandle binding)
{
	UpdateGLBuffer(binding.idx, data, offset, size);
}

void Renderer_GL::DestroyUniformBuffer(UniformBufferHandle handle)
{
...
void Renderer_GL::DestroyShaderBuffer(ShaderBufferHandle handle)
{
	DeleteCommand command{};
	command.mType = DeleteCommand::Type::Buffer;
	command.mHandle = handle.idx;
...
VertexBufferHandle Renderer_GL::CreateVertexBuffer(const void* data, u32 size, BufferUsage usage)
{
	return { CreateGLBuffer(data, size, usage) };
}

void Renderer_GL::UpdateVertexBuffer(VertexBufferHandle handle, const void* data, u32 size, u32 offset)
{
	UpdateGLBuffer(handle.idx, data, offset, size);
}

void Renderer_GL::UpdateIndexBuffer(IndexBufferHandle handle, const void* data, u32 size, u32 offset)
{
	UpdateGLBuffer(handle.idx, data, offset, size);
}

void Renderer_GL::DestroyVertexBuffer(VertexBufferHandle handle)
{
...
IndexBufferHandle Renderer_GL::CreateIndexBuffer(const void* data, u32 size, BufferUsage usage)
{
	return { CreateGLBuffer(data, size, usage) };
}

void Renderer_GL::DestroyIndexBuffer(IndexBufferHandle handle)
{
	DeleteCommand command{};
	command.mType = DeleteCommand::Type::Buffer;
	command.mHandle = handle.idx;
...
TextureHandle Renderer_GL::CreateCubemap(const CubemapDescription& desc)
{
...
TextureHandle Renderer_GL::CreateTexture2D(const TextureDescription2D& desc)
{
...
void Renderer_GL::DestroyTexture(TextureHandle handle)
{
	DeleteCommand command{};
...
void Renderer_GL::GenerateMipMaps(TextureHandle handle)
{
...
TextureHandle Renderer_GL::CreateTexture3D(const TextureDescription3D& desc)
{
	DEBUG_ASSERT(desc.mFormat != TextureFormat::INVALID, "Invalid texture format!");
...
FrameBuffer Renderer_GL::CreateFramebuffer(const FrameBufferDescription& desc)
{
...
FrameBuffer Renderer_GL::CreateFramebuffer(const TextureDescription2D& desc, FramebufferAttachment attachment)
{
...
void Renderer_GL::DestroyFramebuffer(FrameBufferHandle handle)
{
...
ShaderHandle Renderer_GL::CreateShader(const ShaderSourceDescription& desc)
{
...
ShaderHandle Renderer_GL::CreateComputeShader(const char* src)
{
	auto createShader = [](GLenum shaderType, const char* src)
	{
		GLuint result = glCreateShader(shaderType);
...
void Renderer_GL::DestroyShader(ShaderHandle shader)
{
	shaders.erase(shader);
	glDeleteProgram(shader.idx);
}

MeshHandle Renderer_GL::CreateMesh(const MeshDescription& desc)
{
...
void Renderer_GL::DestroyMesh(const MeshHandle mesh)
{
	DeleteCommand command;
...
void Renderer_GL::DrawMesh(MeshHandle mesh, const RenderState& state, std::function<void()> preAction)
{
...
void Renderer_GL::DrawMeshInstanced(MeshHandle mesh, const RenderState& state, VertexBufferHandle data, u32 instanceCount, std::function<void()> preAction)
{
	DEBUG_ASSERT(buildingFrame, "Cannot submit draw if a frame is not in flight");
	DEBUG_ASSERT(state.mRenderPass != std::numeric_limits<u8>::max(), "Invalid render pass");
	DEBUG_ASSERT(state.mShader.idx, "invalid shader!");
...
void Renderer_GL::DispatchCompute(const RenderState& state, u16 groupsX, u16 groupsY, u16 groupsZ, std::function<void()> preAction)
{
	DEBUG_ASSERT(buildingFrame, "Cannot submit draw if a frame is not in flight");
	DEBUG_ASSERT(state.mRenderPass != std::numeric_limits<u8>::max(), "Invalid render pass");
	DEBUG_ASSERT(state.mShader.idx, "invalid shader!");
...
void Renderer_GL::IssueMemoryBarrier()
{
