...
void RenderSystem::Tick(scene::Scene& scene, float dt)
{
	...
	ProcessPointLights(scene, *camera);
...
void RenderSystem::ProcessPointLights(scene::Scene& scene, const Camera& cam)
{
	mLightBinning.ProcessPointLights(scene, cam, mResolution.x, mResolution.y, *mEncoder);
}
...
void RenderSystem::generateSSAO()
{
...
void RenderSystem::ResolveGBuffer()
{
	...
	state.SetUniformBlock("LightSpaceMatrices_UBO", mLightMatricesBuffer);
	state.SetUniformBlock("ShadowPages_UBO", mShadowPagesBuffer);
	state.SetStorageBlock("LightBins_UBO", mLightBinning.GetLightBins());
	state.SetStorageBlock("LightBinIndices_UBO", mLightBinning.getLightBinIndices());
